    ...
    <Stack p="md" h="100%">
      ...
      {batteryTitle && (
        <CustomCard
          ...
          info={
            batteryTitle === 'PCS' &&
            'The highest level aggregation of SOC data at this project is generated by the PCS. PCS SOC represents the average SOC of all BESS equipment connected to the PCS.'
          }
          style={{ flex: 1, minHeight: '250px' }}
          headerChildren={
            <SegmentedControl
...
function getBatteryTitle(usedSensorTypeIds: number[] | undefined) {
  ...
  if (usedSensorTypeIds.includes(SensorTypeEnum.BESS_PCS_SOC_PERCENT)) {
    return 'PCS'
  } else if (
    usedSensorTypeIds.includes(SensorTypeEnum.BESS_ENCLOSURE_SOC_PERCENT)
  ) {
