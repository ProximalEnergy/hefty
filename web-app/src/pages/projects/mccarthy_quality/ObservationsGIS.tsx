...
export default function ObservationsGIS() {
  ...
  if (!devices.data || !observations.data) {
    return <PageLoader />
  }
  ...
  const dataObservations = {
    ...
    features: observations.data.map((observation) => {
      const device = deviceMap[observation.device_id]
      return {
        type: 'Feature',
        properties: {
          device: device,
                ...
                <ScrollArea.Autosize mah={250} maw={500}>
                  <List>
                    {clusterFeatures.slice(0, 25).map((feature, index) => (
                      <List.Item key={index}>
                        {feature.properties?.observation.type}
                      </List.Item>
                    ))}
                  </List>
                </ScrollArea.Autosize>
