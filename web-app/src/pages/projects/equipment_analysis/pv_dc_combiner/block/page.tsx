...
const Page = () => {
  ...
  let data = combinerTags.data
    ?.filter((d) => {
      if (!pcsDeviceIds || !d.device_device_id_path) return false
      const idPathNums = d.device_device_id_path
        .split('.')
        .map((n) => Number(n))
      return idPathNums.some((id) => pcsDeviceIds.includes(id))
    })
    .map((d) => ({
      combiner_device_id: d.device_device_id || -1,
      combiner_dc_capacity: d.device_capacity_dc ?? 0,
      combiner_name_long: d.device_name_long ?? '',
