...
import { Box, Group, Indicator, SegmentedControl, Stack } from '@mantine/core'
import { useState } from 'react'
import { EventAriaRecommendation } from '@/features/project-impacts/components/EventAriaRecommendation'
import { EventChatShim as EventChat } from '@/features/project-impacts/components/EventChatShim'
import { EventGIS } from '@/features/project-impacts/components/EventGIS'
...
import { useEventGeneralViewModel } from '@/features/project-impacts/hooks/use-event-general-view-model'

type EventGeneralViewProps = {
  event: Event
  eventId: number
...
export function EventGeneralView({
  event,
  eventId,
  eventMessageCount,
  project,
  projectId,
}: EventGeneralViewProps) {
  ...
  const viewModel = useEventGeneralViewModel({
    event,
    eventId,
    project,
    projectId,
  })
  const dailyLoss = Number(viewModel.losses.financial.value || 0)
          ...
          <Group align="stretch" flex={2}>
            <Stack>
              ...
              <EventLosses
                ...
                capacityOnly={viewModel.capacityOnly}
              />
            </Stack>
            <CustomCard
              ...
              style={{ height: '100%' }}
            >
              <EventAriaRecommendation event={event} dailyLoss={dailyLoss} />
            </CustomCard>
            <Box
              style={{
                flex: 1,
            ...
            >
              <EventGIS deviceId={event.device_id} />
            </Box>
          </Group>
          <CustomCard
          ...
          >
            {largeCardView === 'traces' && (
              <EventTraces
                ...
                error={viewModel.eventTraceError}
