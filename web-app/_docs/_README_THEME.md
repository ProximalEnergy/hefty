...
2. Save the logo as `logo_<company_name_short>.<ext>` in the `company-logos` folder in the `proximal-static-assets` S3 bucket ([link](https://us-east-2.console.aws.amazon.com/s3/buckets/proximal-static-assets?region=us-east-2&prefix=company-logos/&showversions=false)) (if you downloaded multiple logos, add a descriptive suffix to the filenames). Note that the `company_name_short` should be the same as what is stored in the database. If the company does not exist in the database, create a new entry, making sure to follow the existing naming convention.
