...
import { useGetDevice, useGetGISTrackerByBlock } from '@/hooks/api'
...
const TrackerPerformance = () => {
  ...
  const { data, isLoading, error } = useGetGISTrackerByBlock({
    ...
    queryOptions: {
      enabled: !!projectId && !!blockId && !!start && !!end,
    },
  })

  const device = useGetDevice({
    pathParams: { projectId: projectId || '-1', deviceId: blockId || '-1' },
  ...
  if (isLoading || project.isLoading || device.isLoading) return <PageLoader />
  ...
  const mapStyleEmpty = project.data.has_tracker_layout === false
    ...
    >
      {data && (
        ...
        >
          <Source type="geojson" data={data}>
            ...
            {showLabels && (
              <Layer
                {...gisUtils.layerLabel({ textField: 'name', textRotate: -90 })}
              />
            )}
          </Source>
          {hoverInfo.feature && (
      ...
      <Box style={{ position: 'absolute', top: 0, left: 0, zIndex: 1 }} p="md">
        <Paper p="xs" withBorder>
          <Stack gap="xs">
            <Title order={3} lh={1}>
              Tracker GIS - Block {device.data?.name_long}
            </Title>
            <Group gap={'xs'}>
              <PositionSetpointSegmentedControl />
              <PositionSetpointHoverCard />
            ...
            <Group gap="xs">
              ...
              />
              <Link
                to={`/projects/${projectId}/gis/tracker${location.search}`}
