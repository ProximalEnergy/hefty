"""
Includes soc related kpi computations including average SOC, resting SOC, and
SOC balance score, Depth of Discharge, and Cycle Count.
"""

import xarray as xr
from core.enumerations import DeviceType
from kpi.base.util import coord
from kpi.domain.bess import cycle_count, soc_balance_score
from kpi.domain.util import daily_mean_across_devices, date_local, diff
from kpi.service.transform.method import Input, method_calc
from kpi.service.transform.schema import CalcSchema
from kpi.service.transform.unary import unary_field
from kpi.workflow.download.device_hierarchy.bess import DownloadDeviceHierarchyBess
from kpi.workflow.transform.bess.clean.workflow import TransformBessClean as Clean
from kpi.workflow.transform.bess.evaluate.evaluate import TransformBessEvaluate as Eval


class TransformBessSummarizeSoc(CalcSchema):
    # blocks, modules (distinct from pcs modules), and enclosures not
    # implemented because they are being deprecated

    # =======================================================
    # Project level
    # =======================================================

    # PROJECT_AVERAGE_SOC_PERCENT (16)
    @method_calc
    def project_avg_soc_d(
        soc: xr.DataArray = Input(Clean.project_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    # PROJECT_AVERAGE_DOD (47)
    @method_calc
    def project_avg_dod_d(
        soc: xr.DataArray = Input(project_avg_soc_d.name),
    ) -> xr.DataArray:
        return 1 - soc

    # PROJECT_RESTING_SOC_PERCENT (10)
    @method_calc
    def project_avg_resting_soc_d(
        soc: xr.DataArray = Input(Eval.project_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    # BESS_PROJECT_CHARGE_CYCLES (94)
    @method_calc
    def project_charge_cycles_d(
        soc: xr.DataArray = Input(Clean.project_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        cycle_delta = diff(soc)
        charging_periods = cycle_delta.where(cycle_delta >= 0)
        return charging_periods.groupby(date_local(date_local_5m)).sum()

    # BESS_PROJECT_DISCHARGE_CYCLES (95)
    @method_calc
    def project_discharge_cycles_d(
        soc: xr.DataArray = Input(Clean.project_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        cycle_delta = diff(soc)
        discharging_periods = -cycle_delta.where(cycle_delta <= 0)
        return discharging_periods.groupby(date_local(date_local_5m)).sum()

    # BESS_PROJECT_STRING_SOC_VARIANCE (118)
    @method_calc
    def project_string_soc_variance_d(
        soc: xr.DataArray = Input(Clean.string_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        min_data_coverage = 0.5
        var_5m = soc.var(dim=coord(DeviceType.BESS_STRING))
        data_coverage = soc.notnull().mean(dim=coord(DeviceType.BESS_STRING))
        var_5m = var_5m.where(data_coverage >= min_data_coverage)
        return var_5m.groupby(date_local(date_local_5m)).mean()

    # PROJECT_STRING_SOC_BALANCE_SCORE (120)
    project_string_soc_balance_score_d = unary_field(
        soc_balance_score,
        project_string_soc_variance_d.name,
    )

    # PROJECT_CYCLE_COUNT (9)
    @method_calc
    def project_cycle_count_d(
        soc: xr.DataArray = Input(Clean.project_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return cycle_count(soc=soc, grouper=date_local(date_local_5m))

    # =======================================================
    # PCS level
    # =======================================================

    # BESS_PCS_STRING_SOC_VARIANCE (119)
    @method_calc
    def pcs_string_soc_variance_d(
        soc: xr.DataArray = Input(Clean.string_soc_5m.name),
        string_to_pcs: xr.DataArray = Input(
            DownloadDeviceHierarchyBess.string_to_pcs.name
        ),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        min_data_coverage = 0.5
        device_grouper = string_to_pcs.rename(coord(DeviceType.BESS_PCS))
        var_5m = soc.groupby(device_grouper).var()
        data_coverage = soc.notnull().groupby(device_grouper).mean()
        var_5m = var_5m.where(data_coverage >= min_data_coverage)
        daily = var_5m.groupby(date_local(date_local_5m)).mean()
        daily_coverage = var_5m.notnull().groupby(date_local(date_local_5m)).mean()
        daily = daily.where(daily_coverage >= min_data_coverage)
        return daily

    @method_calc
    def project_avg_pcs_string_soc_variance_d(
        pcs_string_soc_var: xr.DataArray = Input(pcs_string_soc_variance_d.name),
    ) -> xr.DataArray:
        return pcs_string_soc_var.mean(dim=coord(DeviceType.BESS_PCS))

    # BESS_PCS_STRING_SOC_BALANCE_SCORE (121)
    pcs_string_soc_balance_score_d = unary_field(
        soc_balance_score,
        pcs_string_soc_variance_d.name,
    )

    project_avg_pcs_string_soc_balance_score_d = unary_field(
        soc_balance_score,
        project_avg_pcs_string_soc_variance_d.name,
    )

    # =======================================================
    # Bank level
    # =======================================================

    # BESS_BANK_AVERAGE_SOC_PERCENT (24)
    @method_calc
    def bank_avg_soc_d(
        soc: xr.DataArray = Input(Clean.bank_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_bank_soc_d(
        soc: xr.DataArray = Input(Clean.bank_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_BANK, date_local_5m=date_local_5m
        )

    # BESS_BANK_RESTING_SOC_PERCENT (29)
    @method_calc
    def bank_avg_resting_soc_d(
        soc: xr.DataArray = Input(Eval.bank_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_bank_resting_soc_d(
        soc: xr.DataArray = Input(Eval.bank_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_BANK, date_local_5m=date_local_5m
        )

    # BESS_BANK_DEPTH_OF_DISCHARGE (26)
    @method_calc
    def bank_avg_dod_d(
        soc: xr.DataArray = Input(bank_avg_soc_d.name),
    ):
        return 1 - soc

    @method_calc
    def project_avg_bank_dod_d(
        soc: xr.DataArray = Input(project_avg_bank_soc_d.name),
    ):
        return 1 - soc

    # BESS_BANK_CYCLE_COUNT (31)
    @method_calc
    def bank_cycle_count_d(
        soc: xr.DataArray = Input(Clean.bank_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return cycle_count(soc=soc, grouper=date_local(date_local_5m))

    @method_calc
    def project_avg_bank_cycle_count_d(
        cycle_count: xr.DataArray = Input(bank_cycle_count_d.name),
    ):
        return cycle_count.mean(dim=coord(DeviceType.BESS_BANK))

    # =======================================================
    # Block level
    # =======================================================

    # BESS_BLOCK_CYCLE_COUNT (11)
    @method_calc
    def block_cycle_count_d(
        soc: xr.DataArray = Input(Clean.block_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return cycle_count(soc=soc, grouper=date_local(date_local_5m))

    @method_calc
    def project_avg_block_cycle_count_d(
        cycle_count: xr.DataArray = Input(block_cycle_count_d.name),
    ):
        return cycle_count.mean(dim=coord(DeviceType.BESS_BLOCK))

    # BESS_BLOCK_RESTING_SOC_PERCENT (12)
    @method_calc
    def block_avg_resting_soc_d(
        soc: xr.DataArray = Input(Eval.block_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_block_resting_soc_d(
        soc: xr.DataArray = Input(Eval.block_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_BLOCK, date_local_5m=date_local_5m
        )

    # BESS_BLOCK_AVERAGE_SOC_PERCENT (15)
    @method_calc
    def block_avg_soc_d(
        soc: xr.DataArray = Input(Clean.block_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_block_soc_d(
        soc: xr.DataArray = Input(Clean.block_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_BLOCK, date_local_5m=date_local_5m
        )

    # =======================================================
    # String level
    # =======================================================

    # BESS_STRING_AVERAGE_SOC_PERCENT (25)
    @method_calc
    def string_avg_soc_d(
        soc: xr.DataArray = Input(Clean.string_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_string_soc_d(
        soc: xr.DataArray = Input(Clean.string_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_STRING, date_local_5m=date_local_5m
        )

    # BESS_STRING_RESTING_SOC_PERCENT (30)
    @method_calc
    def string_avg_resting_soc_d(
        soc: xr.DataArray = Input(Eval.string_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return soc.groupby(date_local(date_local_5m)).mean()

    @method_calc
    def project_avg_string_resting_soc_d(
        soc: xr.DataArray = Input(Eval.string_resting_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return daily_mean_across_devices(
            value=soc, device_type=DeviceType.BESS_STRING, date_local_5m=date_local_5m
        )

    # BESS_STRING_DEPTH_OF_DISCHARGE (27)
    @method_calc
    def string_avg_dod_d(
        soc: xr.DataArray = Input(string_avg_soc_d.name),
    ):
        return 1 - soc

    @method_calc
    def project_avg_string_dod_d(
        soc: xr.DataArray = Input(project_avg_string_soc_d.name),
    ):
        return 1 - soc

    # BESS_STRING_CYCLE_COUNT (32)
    @method_calc
    def string_cycle_count_d(
        soc: xr.DataArray = Input(Clean.string_soc_5m.name),
        date_local_5m: xr.DataArray = Input(Eval.date_local_5m.name),
    ) -> xr.DataArray:
        return cycle_count(soc=soc, grouper=date_local(date_local_5m))

    @method_calc
    def project_avg_string_cycle_count_d(
        cycle_count: xr.DataArray = Input(string_cycle_count_d.name),
    ):
        return cycle_count.mean(dim=coord(DeviceType.BESS_STRING))
