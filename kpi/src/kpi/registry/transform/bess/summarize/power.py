...
class TransformBessSummarizePower(FieldRegistry[CalcProtocol]):
    ...
    project_avg_c_rate_d = calc_field(resample_mean)(
        x=required(Eval.project_c_rate_5m), grouper=grouper(Eval.date_local_5m)
    )
    ...
    pcs_avg_c_rate_d = calc_field(resample_mean)(
        x=required(Eval.pcs_c_rate_5m), grouper=grouper(Eval.date_local_5m)
    )
    ...
    string_avg_c_rate_d = calc_field(resample_mean)(
        x=required(Eval.string_c_rate_5m), grouper=grouper(Eval.date_local_5m)
    )
