...
def get_time_dimension(x: xr.DataArray) -> Time:
    time_dims = [str(dim) for dim in x.dims if dim in Time]
    if len(time_dims) > 1:
        raise ValueError(f"Multiple time dimensions found: {time_dims}")
    if len(time_dims) == 0:
        raise ValueError("No time dimension found")
    return Time(time_dims[0])
...
def xarray_device_attributes_to_pandas(x: xr.DataArray) -> pd.Series:
    if len(x.dims) > 1:
        raise ValueError(
            f"Too many dimensions for device attributes: {x.dims} in xr.DataArray {x.name}"
        )
    device_dim = get_device_dimension(x)
