...
This skill is the Codex CLI counterpart to the Claude Code skill at `.claude/skills/bulletproof-react-feature-structure/SKILL.md` and the Cursor rule at `web-app/.cursor/rules/bulletproof-react-feature-structure.mdc`. Same source of truth across all three surfaces; the conformance script enforces structure regardless of which tool wrote the code.
