...
7. **Public API.** Do not add `index.ts`. The route entry file is the only
   importable feature surface.
8. **Shim the legacy page.** Replace the body of the original `pages/.../page.tsx` with:
   ```tsx
   import { <Feature>Route } from '@/features/<group>/<feature>/routes/<Feature>Route'
...
| "Adding `index.ts` makes imports shorter." | Feature root barrels are not allowed. Route shims import the route entry file directly. |
...
- The new feature has no `index.ts` barrel.
