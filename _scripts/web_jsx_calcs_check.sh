...
if [ "${RUN_ALL_FILES}" = "true" ]; then
    target_web_files=$(
        rg --files web-app | grep -E '\.(jsx|tsx)$' || true
    )
else
    base_ref="dev"
...
if [ -z "${target_web_files}" ]; then
    if [ "${RUN_ALL_FILES}" = "true" ]; then
        echo "No JSX/TSX files for JSX calculation check; skipping."
    else
...
if [ "${#targets[@]}" -eq 0 ]; then
    if [ "${RUN_ALL_FILES}" = "true" ]; then
        echo "No existing JSX/TSX files for JSX calculation check; skipping."
    else
