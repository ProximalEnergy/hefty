...
def get_emails_from_clerk(user_ids: list[str], api_prod: bool) -> dict[str, str]:
    ...
    if not clerk_secret_key:
        raise ValueError(f"Clerk secret key not set for {'production' if api_prod else 'development'} environment")
        ...
        for user_id in batch:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Error fetching user {user_id}: {response.status_code} - {response.text}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Export all user emails to text file (for BCC field)")
    parser.add_argument(
        "--output",
        "-o",
