...
# Keep uv.lock current before Docker runs uv export --frozen
uv sync --project "$MONO_ROOT/microservices/kpi-pipeline-fetcher"
...
docker buildx build \
  ...
  --provenance=false \
  --push \
  -f "$MONO_ROOT/microservices/kpi-pipeline-fetcher/Dockerfile" \
  -t "$ECR_URI" \
  "$MONO_ROOT"
